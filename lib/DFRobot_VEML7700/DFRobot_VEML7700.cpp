...
uint8_t DFRobot_VEML7700::sendData(uint8_t command, uint32_t data)
{
  ...
  if (Wire1.write(command) != 1) {
    return STATUS_ERROR;
  }
  if (Wire1.write(uint8_t(data & 0xff)) != 1) {
    return STATUS_ERROR;
  }
  if (Wire1.write(uint8_t(data >> 8)) != 1) {
    return STATUS_ERROR;
  }
...
uint8_t DFRobot_VEML7700::receiveData(uint8_t command, uint32_t& data)
{
  ...
  if (Wire1.endTransmission(false)) {  // NB: don't send stop here
    return STATUS_ERROR;
  }
  if (Wire1.requestFrom(uint8_t(I2C_ADDRESS), uint8_t(2)) != 2) {
    return STATUS_ERROR;
  }
  data = Wire1.read();
